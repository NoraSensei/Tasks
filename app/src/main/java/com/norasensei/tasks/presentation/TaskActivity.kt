...
import com.norasensei.tasks.databinding.ActivityTaskBinding
import com.norasensei.tasks.domain.models.Task
import com.norasensei.tasks.presentattion.viewModel.TaskViewModel
import java.util.*
...
class TaskActivity : AppCompatActivity() {
	...
	override fun onCreate(savedInstanceState: Bundle?) {
		...
		binding.add.setOnClickListener {
			...
			if (name.isNotEmpty()) {
				taskViewModel.addTask(Task(name, "", taskListID, isChecked))
				finish()
			}
		}
		...
		binding.save.setOnClickListener {
			...
			if (name.isNotEmpty()) {
				taskViewModel.updateTask(
					Task(name, "", taskListID, isChecked, taskID)
				)
				finish()
			}
		}
